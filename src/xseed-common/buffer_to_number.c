    ...
    switch(type)
    {
        ...
        case XSEED_FLOAT:
            ...
            *((float *)output) = u1.f;
            break;
        case XSEED_DOUBLE:
            u2.i = buffer[0] | (uint64_t) buffer[1] << 8 | (uint64_t) buffer[2] << 16 | (uint64_t) buffer[3] << 24 | (uint64_t) buffer[4] << 32 | (uint64_t) buffer[5] << 40 | (uint64_t) buffer[6] << 48 | (uint64_t) buffer[7] << 56;
            *((double *)output) = u2.d;
            break;
        case XSEED_STEIM1:
        case XSEED_STEIM2:
